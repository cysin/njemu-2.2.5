...
static int build_zoom_tables(void)
{
	...
	if ((zoom_tables_tmp = memalign(MEM_ALIGN, 0x10000)) == NULL)
	{
		error_memory("ZOOM_TABLES");
		return 0;
	}
	memset(zoom_tables_tmp, 0, 0x10000);

	skip_fullmode0 = &zoom_tables_tmp[0x100*0x40*0];
	tile_fullmode0 = &zoom_tables_tmp[0x100*0x40*1];
	skip_fullmode1 = &zoom_tables_tmp[0x100*0x40*2];
	tile_fullmode1 = &zoom_tables_tmp[0x100*0x40*3];
	...
	memcpy(memory_region_gfx4, zoom_tables_tmp, 0x10000);

	free(zoom_tables_tmp);
