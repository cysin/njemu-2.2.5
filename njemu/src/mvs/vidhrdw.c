...
void neogeo_video_init(void)
{
	...
	skip_fullmode0 = &memory_region_gfx4[0x100*0x40*0];
	tile_fullmode0 = &memory_region_gfx4[0x100*0x40*1];
	skip_fullmode1 = &memory_region_gfx4[0x100*0x40*2];
	tile_fullmode1 = &memory_region_gfx4[0x100*0x40*3];
...
void neogeo_screenrefresh(void)
{
	if (next_update_first_line <= LAST_VISIBLE_LINE)
	{
		...
		draw_sprites(next_update_first_line, LAST_VISIBLE_LINE);
	}
...
void neogeo_partial_screenrefresh(int current_line)
{
	if (current_line >= FIRST_VISIBLE_LINE)
	{
		if (current_line >= next_update_first_line)
		{
			...
			draw_sprites(next_update_first_line, current_line);
		}
